...
`X-Shopify-Hmac-Sha256` is the **signature verification header for Shopify Webhooks**, used to ensure that webhook requests actually originate from Shopify's official servers rather than being spoofed by third parties.

It is a Base64-encoded string that Shopify generates for each webhook request by signing the request body with the **HMAC-SHA256** algorithm.
...
|---------------|-------------|-------------|
| **Forged Request** | An attacker directly calls your webhook endpoint with fabricated order data | Members are erroneously registered and points are fraudulently awarded |
| **Replay Attack** | Legitimate requests are intercepted and resent repeatedly | The same order earns points multiple times |
| **Data Tampering** | A man-in-the-middle modifies request content | A $10 order is changed to $1000, awarding extra points |
| **Exposed Endpoint** | Webhook URLs are public and accessible to anyone | Unverified requests directly manipulate business data |

The core value of HMAC signature verification: **only parties holding the same Secret can generate matching signatures**, which attackers cannot forge.
...
sequenceDiagram
    ...
    Shopify->>Shopify: 4. Base64-encode the signature
    ...
    App->>App: 9. Base64-encode the result
...
sequenceDiagram
    ...
    Shopify->>Shopify: 5. Base64-encode the signature
    ...
    App->>App: 10. Base64-encode the result
